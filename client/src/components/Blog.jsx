          ...
          <div className="w-full lg:w-2/3 space-y-16">
            {blogPosts.map((post) => (
              <div key={post.id} className="bg-white p-4 shadow rounded-md">
                <Link to="/blog-detail" className="relative block">
                  <img src={post.image} alt={post.title} className="rounded-md w-full h-auto" />
                ...
                <div className="pt-6">
                  <h3 className="text-xl font-semibold mb-4">
                    <Link to="/blog-detail" className="hover:text-blue-600">
                      {post.title}
                    </Link>
                  </h3>
                  <p className="text-gray-600 text-sm mb-4">{post.desc}</p>
                  <div className="flex justify-between text-sm text-gray-500">
                    <span>By Admin | StreetStyle, Fashion, Couple | 8 Comments</span>
                    <Link to="/blog-detail" className="text-blue-600 hover:underline">Continue Reading &rarr;</Link>
                  </div>
                </div>
              </div>
            ))}
